...
def get_cin_tudata(data,batch_size):
    ...
    for idx in split_idx['train']:
        ...
        new_train_list.append(idx)


    for idx in split_idx['valid']:
        if idx in [62,94,99]: continue
        new_val_list.append(idx)


    for idx in split_idx['test']:
        if idx in [31,34,59,62,66,100,101]: continue
        new_test_list.append(idx)

    org_train = [dats for dats in original_data[np.array(new_train_list)]]
    org_val = [dats for dats in original_data[np.array(new_val_list)]]
    org_test = [dats for dats in original_data[np.array(new_test_list)]]


    #breakpoint()
    train_loader = DataLoader(datasets[np.array(new_train_list)], batch_size=batch_size, shuffle=False,max_dim=datasets.max_dim)
    val_loader = DataLoader(datasets[np.array(new_val_list)], batch_size=len(new_val_list), shuffle=False,max_dim=datasets.max_dim)
    test_loader = DataLoader(datasets[np.array(new_test_list)], batch_size=len(new_test_list), shuffle=False,max_dim=datasets.max_dim)
    ...
    for idx,sample in enumerate(org_train):
    ...
    for idx,sample in enumerate(org_val):
        #cycles = nx.cycle_basis(to_networkx(sample,to_undirected=True))
        #print(idx)
        #if idx in [62,94,99]: continue
        ph_feats = create_landscapes_gudhi(_forman_curvature_unweighted, to_networkx(sample,to_undirected=True), hom_deg=2, exact=True)
    ...
    for idx,sample in enumerate(org_test):
        #cycles = nx.cycle_basis(to_networkx(sample,to_undirected=True))
        #print(idx)
        #if idx in [31,34,59,62,66,100,101]: continue
        ph_feats = create_landscapes_gudhi(_forman_curvature_unweighted, to_networkx(sample,to_undirected=True), hom_deg=2, exact=True)
    ...
    org_train_loader = PyGDataloader(new_train,batch_size=batch_size,shuffle=False)
    org_val_loader = PyGDataloader(new_val,batch_size=len(new_val_list),shuffle=False)
    org_test_loader = PyGDataloader(new_test,batch_size=len(new_test_list),shuffle=False)
...
def calculate_persistent_homology(G,w, k=3):
