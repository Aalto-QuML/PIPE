...
def create_landscapes_gudhi(curvature_fn, graphs, hom_deg, exact=True):
    ...
    def landscape_iterator(graph, exact=True):
        ...
        diagrams = calculate_persistent_homology(graph, hom_deg)

        concat_diag = []
        for i in diagrams:
            if len(i)>0:
                for j in i:
                    concat_diag.append(j)

        p_diagrams = np.array(concat_diag)
        #if len(diagrams)>0:
        #    p_diagrams = np.concatenate(
        #        (np.array(diagrams[0]), np.array(diagrams[1]), np.array(diagrams[2])), axis=0
        #    )
        #else:
    ...
    all_landscapes = landscape_iterator(graphs)
...
def calculate_persistent_homology(G, k):
