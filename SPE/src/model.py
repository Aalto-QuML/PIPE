...
class Model(nn.Module):
    ...
    def forward(self, batch: Batch) -> torch.Tensor:
        ...
        if self.positional_encoding is not None:
            ...
            PE,topo_emb = self.positional_encoding(batch.Lambda, eig_mats, batch.edge_index, batch.batch,batch)   # [N_sum, D_pe]
...
class GINEBaseModel(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = create_mlp(hidden_dims + 64, target_dim)
    ...
    def forward(
        ...
        Y_pred = self.mlp(torch.cat([X_n,topo_emb],dim=-1))        # [B, 1]
