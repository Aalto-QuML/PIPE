...
from src.gin import GIN,GIN_PH
...
class StableExpressivePE(nn.Module):
    ...
    def forward(self, Lambda, V, edge_index, batch,data) -> torch.Tensor:
        """
...
#                            for i in range(37)], dim=-1)
        ...
        return self.phi(W_list, edge_index,data)   # [N_sum, D_pe]
...
class GINPhi(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.gin = GIN_PH(n_layers, in_dims, hidden_dims, out_dims, create_mlp, bn)
        self.mlp = create_mlp(out_dims, out_dims)

    def forward(self, W_list: List[torch.Tensor], edge_index: torch.Tensor, batch) -> torch.Tensor:
        """
        :param W_list: The {V * psi_l(Lambda) * V^T: l in [m]} tensors. [N_i, N_i, M] * B
        :param edge_index: Graph connectivity in COO format. [2, E_sum]
        :return: Positional encoding matrix. [N_sum, D_pe]
        ...
        W = torch.cat(W_pad_list, dim=0)   # [N_sum, N_max, M]
        mask = torch.cat(mask, dim=0)   # [N_sum, N_max]
        PE,topo_emb = self.gin(W, edge_index,batch,mask)       # [N_sum, N_max, D_pe]
        PE = (PE * mask.unsqueeze(-1)).sum(dim=1)
        return PE,topo_emb               # [N_sum, D_pe]
        # return PE.sum(dim=1)
